...
const HeadControllerForm: FC<IPropsForm<IHeadController>> = ({ data: headController, isEdited, setIsEdited, toggleModal }) => {
	const { register, handleSubmit, formState: { errors, isValid }, reset } = useForm<THeadControllerData>({
		...
		defaultValues: {
			...
			actualFirmwareVersion: headController?.actualFirmwareVersion
		}
	})
	...
	return (
		<div className="work-log__form">
			...
			<form className="form" onSubmit={handleSubmit(isEdited ? submitUpdate : submitCreate)}>
				...
				<Group>
					<Input
						label='Актуальная версия прошивки контроллера'
						name='actualFirmwareVersion'
						register={register}
						errorMessage={errors.actualFirmwareVersion?.message}
						placeholder='Введите актуальную версию прошивки...'
					/>
				</Group>
